...
class TokenPermuter:
    ...
    def __init__(self, tokenizer, corpus):    
        tokenized = tokenize(corpus, 'nld_Latn', tokenizer, max_length=128)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--parallel", type=bool, required=True, help="If artificual langs should be parallel to src lang")
    parser.add_argument("--output_file", type=str, required=True, help="The output CSV filename")
    args = parser.parse_args()
    base_model = "facebook/nllb-200-distilled-600M"
    src_file = 'data/eng-dutch/Europarl.en-nl.en'
    tgt_file = 'data/eng-dutch/Europarl.en-nl.nl'
    ...
    tokenized = tokenize(tgt_sents, 'nld_Latn', tokenizer, max_length=128)
    ids = [idx for idx in tokenized['input_ids'].unique().tolist() if 4 <= idx <= 256000]
